...
export default function CommunityQuestionsAnswers({ selectedProduct }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    payload.append("message", formData.comments);
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const user = {
          id: 0,
          name: formData.full_name || "You",
          profile_photo_url: `/users.png`,
        };
        ...
        // Reset form
        setFormData({ full_name: "", email_address: "", comments: "" });
        setParentId(null);
        setFormHeading("Ask a Question");
      } else {
        setError(res.data.message || "Failed to post");
      }
    } catch (err: any) {
      ...
      >
        ...
        <div className="flex xl:flex-row lg:flex-row md:flex-row min-[360px]:flex-col gap-5 xl:justify-between lg:justify-between md:justify-between min-[360px]:justify-start mt-5 mb-2">
          <div className="likeDislike flex gap-5">
            <span className="transition-all border-1 border-[#98C1A9] py-2 min-w-[100px] w-full rounded-xl cursor-pointer flex justify-center items-center gap-2 text-[#666664] font-medium text-sm">
            ...
            <div className="field flex flex-col gap-2 w-full">
              ...
              <div className="w-full relative flex items-center">
                <input
                  ...
                  readOnly
                  value={
                    questions.find((q) => q.id === parentId)?.message || ""
                  }
